function SingleImageUpload(id, options) {
    this.me = $(id); var defaultOpt = { boxWidth: '200px', boxHeight: 'auto' }
    this.preview = $('<div id="preview"><img src="../../images/lib/singleupload/upload.png" class="img-responsive"  style="width: 100%;height: auto;" alt="" title=""></div>'); this.opts = $.extend(true, defaultOpt, {}, options); this.init(); this.callback = this.opts.callback;
}
SingleImageUpload.prototype = {
    init: function () { this.me.append(this.preview); this.me.append(this.fileupload); this.cssInit(); this.eventClickInit(); },
    cssInit: function () {
    ...
    onDragover: function (e) { e.stopPropagation(); e.preventDefault(); e.dataTransfer.dropEffect = 'copy'; },
    onDrop: function (e) {
    ...
    onChangeUploadFile: function () {
        var fileInput = this.fileInput; var files = fileInput.files; var file = files[0];
        var img = window.URL.createObjectURL(file); var filename = file.name; this.me.find("img").attr("src", img); this.me.find("img").attr("title", filename);
    ...
    createImageUploadDialog: function () {
        ...
        if (!fileInput) {
            ...
            fileInput.type = 'file';
            fileInput.name = 'ime-images'; fileInput.multiple = true;
    ...
    src: function (uri) {
        if (uri)
            this.preview.find("img").attr("src", uri);
        else
            this.preview.find("img").attr("src", "../../images/lib/singleupload/upload.png");
    }
}